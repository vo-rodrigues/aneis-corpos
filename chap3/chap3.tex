...
\begin{definition}[Subanel]
    Seja $A$ um anel e $B\subseteq A$.
    Dizemos que $B$ é um \emph{subanel} de $A$ se:
    
    \begin{itemize}
        \item $0_A, 1_A \in B$
        \item Para todos $a, b \in B$, $a+b \in B$.
        \item Para todos $a, b \in B$, $ab\in B$
        \item Para todo $a \in B$, $-a \in B$.
    \end{itemize}

\end{definition}

O ponto chave da definição é que:

\begin{prop}[Subanel]
    Seja $A$ um anel e $B$ um subanel de $A$. Então $(B, +|_{B^2}, \cdot|_{B^2}, 0_B, 1_B)$ é um anel, onde $+|_{B^2}:B^2\rightarrow B$ e $\cdot|_{B^2}:B^2\rightarrow B$ são as restrições das operações de $A$ À $B^2$.
