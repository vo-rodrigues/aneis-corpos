...
\begin{lemma}
Seja $R$ um domínio de integridade.
Se $r \in R$ é irredutível e $r$ se escreve como um produto de primos, então $r$ é primo.
\end{lemma}
\begin{proof}
Suponha que $r=p_1\cdots p_n$ com $p_1, \dots, p_n$ primos.
Procedemos por indução em $n$.

Para $n=1$ é trivial.
Para o passo indutivo, se $r=p_1\cdots p_np_{n+1}$, então $p_1\cdots p_n$ é invertível ou $p_{n+1}$ é invertível.
Temos que $p_{n+1}$ não é invertível, logo, $p_1\cdots p_n$ é invertível, e, portanto, $r$ é associado ao primo $p_{n+1}$, e, portanto, é primo.
\end{proof}
\begin{prop}
Seja $R$ um domínio de integridade.
São equivalentes:
...
\begin{proof}
Resta provar apenas que b) implica a), e, para isso, basta 
\end{proof}
