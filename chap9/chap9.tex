...
\end{proof}
\section{O critério da irredutibilidade de Eisenstein}
    ...
    Observação: se escrevermos $p=\sum_{i=0}^n a_i x^i$ com $n>\gr p$ adicionando coeficientes $0$ à escrita, então $\MDC(a_0, \dots, a_n)=C(p)$, pois para qualquer conjunto finito $A$, $\MDC(A)=\MDC(A\cup \{0\})$.

    \begin{lemma}[Gauss]
        Seja $R$ um domínio de fatoração única e $p, q \in R[x]\setminus\{0\}$. Então $C(pq)=\{ab: a \in C(p), b \in C(q)=C(p)C(q)$.
    ...
    \begin{proof}
        ...
        Dado $i\leq n+m$ e $j\leq i$, temos $j\leq 0$, temos que $a|a_{i-j}$ e $b|b_j$, logo, $ab|a_{i-j}b_j$, logo, $ab|\left(\sum_{j=0}^{i}a_{i-j}b_j\right)=c_i$.

        Para cada $i\leq m+n$, existe $a_i'$ e $b_i'$ tais que $a_i=aa_i'$ e $b_i=bb'_i$.
        Então, para cada $i\leq m+n$, $c_i=ab\sum_{i=0}^{n+m}a_{i-j}'b_j'$.
        Seja $c_i'=\sum_{i=0}^{n+m}a_{i-j}'b_j'$.

        Segue que $\MDC(c_0, \dots, c_{n+m})=\MDC(abc'_0, \dots,abc_{n+m}')=ab\MDC(c'_0, \dots, c_{n+m}')$. Assim, basta ver que $1 \in \MDC(c'_0, \dots, c_{n+m}')$. Para tanto, basta ver que se $p \in R$ é primo, então $p$ não é divisor comum de $(c'_0, \dots, c'_{n+m})$.
        Fixe $p$.

        Note que: 
        \begin{equation*}
        a\in \MDC(a_0, \dots, a_{n+m})=\MDC(a_0, \dots, a_n)=\MDC(aa_0', \dots, aa'_n)=a\MDC(a_0', \dots, a'_n).
        \end{equation*}
        Logo, $1 \in \MDC(a'_0, \dots, a'_n)$. Seja $r$ o primeiro inteiro tal que $p\not|a'_r$.

        Analogamente, $1 \in \MDC(b'_0, \dots, b'_m)$. Seja $s$ o primeiro inteiro tal que $p\not|b'_s$.

        Temos que se $p\not| c'_{r+s}$: com efeito, $c'_{r+s}=\sum(a_{r+s-j}b_j:0\leq j\leq r+s, j\neq s)+a_rb_s$. Temos que $p|\sum(a_{r+s-j}b_j:0\leq j\leq r+s, j\neq s)$, pois divide cada somando, mas $p\not |a_rb_s$ pois $p$ é primo, deveríamos ter $p|a_r$ ou $p|a_s$. Assim, $p\not | c'_{r+s}$.
    \end{proof}
\section{Exercícios}
\begin{exer}
