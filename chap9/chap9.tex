...
\begin{prop}
Seja $R$ um anel comutativo infinito e $f, g$ funções polinomiais dadas por $f(x)=\sum_{i=0}^n a_ix^i$ e $g(x)=\sum_{i=0}^m b_ix^i$ com $a_n, b_m\neq 0$. Então $f, g$ não são nulas, e $f=g$ se, e somente se, $a_i=b_i$ para todo $i\leq \max\{n, m\}$.
\end{prop}

\begin{proof}
    Olhando os coeficientes de $h=f-g$, basta provar que se os coeficientes de uma função polinomial são não nulos, então a função polinomial não é nula.
