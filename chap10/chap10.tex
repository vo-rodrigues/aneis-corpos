...
\begin{prop}\label{prop:constritíveisSequencia}
...
\begin{exer}Seja $F/E$ uma extensão de corpos. Mostre que $[F:E]=1$ se, e somente se $F=E$.
\end{exer}
...
\end{exer}
\begin{exer}
Seja $L$ um subcorpo de $\mathbb R$. Mostre que se $a \in \mathbb R$ é positivo, então $a$ é algébrico sobre $L$ se, e somente se $\sqrt a$ é algébrico sobre $L$.
\end{exer}
\begin{exer}
    Seja $F/E$ uma extensão de corpos de característica $0$ e $p \in E[x]$ irredutível.
    Mostre que se $\alpha \in F$ é tal que $p(\alpha)=0$, então $(x-\alpha)^2\nmid p(x)$.
\end{exer}


\begin{exer}[Relações de Girard] Seja $K$ um corpo.
...
\begin{exer}
Seja $F$ um corpo. Mostre que o conjunto $G$ dos automorfismos de $F$ em $F$ é, com a composição e a função identidade, um grupo.
